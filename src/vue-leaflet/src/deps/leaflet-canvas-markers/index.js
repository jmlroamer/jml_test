...
const CanvasIconLayer = L.Layer.extend({
	...
	initialize: function (options) {
		Object.assign(this.options.labelStyle, options.labelStyle);
		this.options = Object.assign(this.options, options, { labelStyle: this.options.labelStyle });
		this._onClickListeners = [];
		this._onHoverListeners = [];
		this._onDrawTextListeners = [];
	},
	...
	onAdd: function (map) {
		...
		if (this.options.pane) pane.appendChild(this._canvas);
	...
	_drawMarker: function (marker, pointPos) {
		...
		if (self.options.enableText) {
			this._onDrawTextListeners.forEach(function (listener) {
				listener(marker, pointPos);
			});
		}
	...
	_drawImage: function (marker, pointPos) {
		...
		const x = pointPos.x - (options.iconAnchor?.[0] ?? 0);
		const y = pointPos.y - (options.iconAnchor?.[1] ?? 0);
		marker._label && marker._showLabel && this._drawText(marker._label, x, y, options.iconSize[0]);
		this._context.drawImage(marker.canvas_img, x, y, options.iconSize[0], options.iconSize[1]);
	},

	drawValue(text, x, y, font, fill, stroke, isStroke = true) {
		const { _context } = this;
		_context.font = font;
	...
	_drawText(text, x, y, size) {
		const {
			_context,
			options: {
				labelStyle: { fontSize, fontColor, strokeStyle }
			}
		} = this;
		const { width } = _context.measureText(text);
		// _context.strokeStyle = strokeStyle;
		_context.strokeStyle = "rgba(255, 240, 172, 0.8)";
		_context.fillStyle = fontColor;
		_context.font = `${fontSize}px Microsoft YaHei`;
		_context.strokeText(text, x - width / 2 + size / 2, y + size + fontSize);
		_context.fillText(text, x - width / 2 + size / 2, y + size + fontSize);
	},
	...
	onDrawTextListener(listener) {
		this._onDrawTextListeners.push(listener);
	},
	...
	removeOnClickListener: function (listener) {
		this._removeListener("_onClickListeners", listener);
	},

	removeOnHoverListener: function (listener) {
		this._removeListener("_onHoverListeners", listener);
	},

	_removeListener(key, listener) {
		const listenerList = this[key];
		const index = listenerList.indexOf(listener);
		index !== -1 && listenerList.splice(index, 1);
	},

	_executeListeners: function (event) {
		if (!this._markers || this._isMove) return;

		var me = this;
		var x = event.containerPoint.x;
		var y = event.containerPoint.y;

		if (me._openToolTip && !me._openToolTip._tooltip?.options?.permanent) {
