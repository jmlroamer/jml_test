<template>
  <div class="box-container">
    ...
    >
      ...
      <el-table-column label="施测时间" align="center" prop="mstm" width="180">
        <template #default="scope">
          <span>{{ scope.row.mstm }}</span>
        </template>
      </el-table-column>
      <el-table-column label="点序号" align="center" prop="ptno" />
      <el-table-column label="库水位" align="center" prop="rz" />
      <el-table-column label="蓄水量" align="center" prop="w" />
      <el-table-column label="水面面积" align="center" prop="wsfa" />
      <el-table-column
        label="操作"
        fixed="right"
        width="155"
    ...
    >
      ...
      >
        <el-form-item label="测站编码" prop="stcd">
          <el-input
            :disabled="dialogTiltle == '修改库(湖)容曲线'"
            v-model="form.stcd"
            placeholder="请输入测站编码"
          />
        </el-form-item>
        ...
        <el-form-item label="库水位" prop="rz">
          <el-input v-model="form.rz" placeholder="请输入库水位" />
        </el-form-item>
        <el-form-item label="蓄水量" prop="w">
          <el-input v-model="form.w" placeholder="请输入蓄水量" />
        </el-form-item>
        <el-form-item label="水面面积" prop="wsfa">
          <el-input v-model="form.wsfa" placeholder="请输入水面面积" />
        </el-form-item>
      ...
      <template #footer>
        <div class="dialog-footer">
          <el-button type="primary" @click="submitForm(YzStZvarlBRef)"
            >确 定</el-button
          >
...
import {
  ...
  addYzStZvarlB,
  updateYzStZvarlB,
  importTemplateYzStZvarlB,
  importYzStZvarlB,
  exportYzStZvarlB,
} from "@/apis/modules/meteorology/YzStZvarlB";

const YzStZvarlBRef = ref<FormInstance>();
const queryRef = ref<FormInstance>();
const upload = ref<UploadInstance>();
const YzStZvarlBList = ref([]);
...
const getList = async () => {
  ...
  const res: any = await listYzStZvarlB(queryParams);
  YzStZvarlBList.value = res.rows;
  if (YzStZvarlBList.value.length > 0) {
    YzStZvarlBList.value.forEach((item: any) => {
...
const handleAdd = () => {
  ...
  dialogTiltle.value = "添加库(湖)容曲线";
};

/** 修改按钮操作 */
const handleUpdate = async (row: any) => {
  resetForm();
  const res: any = await getYzStZvarlB(row);
  form.value = res.data;
  dialogOpen.value = true;
  dialogTiltle.value = "修改库(湖)容曲线";
};
...
const submitForm = (formEl: FormInstance | undefined) => {
  ...
  formEl.validate(async (valid) => {
    if (valid) {
      if (dialogTiltle.value == "修改库(湖)容曲线") {
        const res: any = await updateYzStZvarlB(form.value);
        if (res.code == 200) {
          ElMessage({
            type: "success",
            message: "修改成功！",
        ...
        const res: any = await addYzStZvarlB(form.value);
        if (res.code == 200) {
          ElMessage({
            type: "success",
            message: "添加成功！",
...
const handleDelete = (row: any) => {
  ...
  ElMessageBox.confirm(
    ...
    const res: any = await delYzStZvarlB(params);
    if (res.code == 200) {
      ElMessage({
        type: "success",
        message: "删除成功！",
...
const handleExport = async () => {
  const res = await exportYzStZvarlB(queryParams);
  saveAsExcelFile(res, `YzStZvarlB_${new Date().getTime()}`);
};
/** 导出模板按钮操作 */
const handleExportTemplate = async () => {
  const res: any = await importTemplateYzStZvarlB({});
...
const handleAddFile = async (ev: any) => {
  ...
  const res: any = await importYzStZvarlB(formData);
  if (res.code == 200) {
    ElMessage({
      type: "success",
      message: "导入成功！",
