    ...
    >
      ...
      <template #footer>
        <div class="dialog-footer">
          <el-button type="primary" @click="submitForm(YzStRsvrRRef)"
            >确 定</el-button
          >
...
import {
  ...
  addYzStRsvrR,
  updateYzStRsvrR,
  importTemplateYzStRsvrR,
  importYzStRsvrR,
  exportYzStRsvrR,
} from "@/apis/modules/meteorology/YzStRsvrR";

const YzStRsvrRRef = ref<FormInstance>();
const queryRef = ref<FormInstance>();
const upload = ref<UploadInstance>();
const YzStRsvrRList = ref([]);
...
const getList = async () => {
  ...
  const res: any = await listYzStRsvrR(queryParams);
  YzStRsvrRList.value = res.rows;
  if (YzStRsvrRList.value.length > 0) {
    YzStRsvrRList.value.forEach((item: any) => {
...
const handleAdd = () => {
  ...
  dialogTiltle.value = "添加水库水情";
};

/** 修改按钮操作 */
const handleUpdate = async (row: any) => {
  resetForm();
  const res: any = await getYzStRsvrR(row);
  form.value = res.data;
  dialogOpen.value = true;
  dialogTiltle.value = "修改水库水情";
};
...
const submitForm = (formEl: FormInstance | undefined) => {
  ...
  formEl.validate(async (valid) => {
    if (valid) {
      if (dialogTiltle.value == "修改水库水情") {
        const res: any = await updateYzStRsvrR(form.value);
        if (res.code == 200) {
          ElMessage({
            type: "success",
            message: "修改成功！",
        ...
        const res: any = await addYzStRsvrR(form.value);
        if (res.code == 200) {
          ElMessage({
            type: "success",
            message: "添加成功！",
...
const handleDelete = (row: any) => {
  ...
  ElMessageBox.confirm(
    ...
    const res: any = await delYzStRsvrR(params);
    if (res.code == 200) {
      ElMessage({
        type: "success",
        message: "删除成功！",
...
const handleExport = async () => {
  const res = await exportYzStRsvrR(queryParams);
  saveAsExcelFile(res, `YzStRsvrR_${new Date().getTime()}`);
};
/** 导出模板按钮操作 */
const handleExportTemplate = async () => {
  const res: any = await importTemplateYzStRsvrR({});
...
const handleAddFile = async (ev: any) => {
  ...
  const res: any = await importYzStRsvrR(formData);
  if (res.code == 200) {
    ElMessage({
      type: "success",
      message: "导入成功！",
