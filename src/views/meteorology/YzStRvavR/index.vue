<template>
  <div class="box-container">
    ...
    >
      ...
      <el-table-column label="平均水位" align="center" prop="avz" />
      <el-table-column label="平均流量" align="center" prop="avq" />
      <el-table-column
        label="操作"
        fixed="right"
        width="155"
    ...
    >
      ...
      >
        <el-form-item label="测站编码" prop="stcd">
          <el-input
            :disabled="dialogTiltle == '修改河道水情多日均值'"
            v-model="form.stcd"
            placeholder="请输入测站编码"
          />
        </el-form-item>
        ...
        <el-form-item label="平均水位" prop="avz">
          <el-input v-model="form.avz" placeholder="请输入平均水位" />
        </el-form-item>
        <el-form-item label="平均流量" prop="avq">
          <el-input v-model="form.avq" placeholder="请输入平均流量" />
        </el-form-item>
      </el-form>
      <template #footer>
        <div class="dialog-footer">
          <el-button type="primary" @click="submitForm(YzStRvavRRef)"
            >确 定</el-button
          >
...
import {
  ...
  addYzStRvavR,
  updateYzStRvavR,
  importTemplateYzStRvavR,
  importYzStRvavR,
  exportYzStRvavR,
} from "@/apis/modules/meteorology/YzStRvavR";

const YzStRvavRRef = ref<FormInstance>();
const queryRef = ref<FormInstance>();
const upload = ref<UploadInstance>();
const YzStRvavRList = ref([]);
...
const getList = async () => {
  ...
  const res: any = await listYzStRvavR(queryParams);
  YzStRvavRList.value = res.rows;
  if (YzStRvavRList.value.length > 0) {
    YzStRvavRList.value.forEach((item: any) => {
...
const handleAdd = () => {
  ...
  dialogTiltle.value = "添加河道水情多日均值";
};

/** 修改按钮操作 */
const handleUpdate = async (row: any) => {
  resetForm();
  const res: any = await getYzStRvavR(row);
  form.value = res.data;
  dialogOpen.value = true;
  dialogTiltle.value = "修改河道水情多日均值";
};
...
const submitForm = (formEl: FormInstance | undefined) => {
  ...
  formEl.validate(async (valid) => {
    if (valid) {
      if (dialogTiltle.value == "修改河道水情多日均值") {
        const res: any = await updateYzStRvavR(form.value);
        if (res.code == 200) {
          ElMessage({
            type: "success",
            message: "修改成功！",
        ...
        const res: any = await addYzStRvavR(form.value);
        if (res.code == 200) {
          ElMessage({
            type: "success",
            message: "添加成功！",
...
const handleDelete = (row: any) => {
  ...
  ElMessageBox.confirm(
    ...
    const res: any = await delYzStRvavR(params);
    if (res.code == 200) {
      ElMessage({
        type: "success",
        message: "删除成功！",
...
const handleExport = async () => {
  const res = await exportYzStRvavR(queryParams);
  saveAsExcelFile(res, `YzStRvavR_${new Date().getTime()}`);
};
/** 导出模板按钮操作 */
const handleExportTemplate = async () => {
  const res: any = await importTemplateYzStRvavR({});
...
const handleAddFile = async (ev: any) => {
  ...
  const res: any = await importYzStRvavR(formData);
  if (res.code == 200) {
    ElMessage({
      type: "success",
      message: "导入成功！",
