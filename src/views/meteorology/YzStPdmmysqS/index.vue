<template>
  <div class="box-container">
    ...
    >
      ...
      <el-table-column label="累计降水量" align="center" prop="accp" />
      <el-table-column
        label="操作"
        fixed="right"
        width="155"
    ...
    >
      ...
      >
        <el-form-item label="测站编码" prop="stcd">
          <el-input
            :disabled="dialogTiltle == '修改旬月降水量系列'"
            v-model="form.stcd"
            placeholder="请输入测站编码"
          />
        </el-form-item>
        ...
        <el-form-item label="累计降水量" prop="accp">
          <el-input v-model="form.accp" placeholder="请输入累计降水量" />
        </el-form-item>
      ...
      <template #footer>
        <div class="dialog-footer">
          <el-button type="primary" @click="submitForm(YzStPdmmysqSRef)"
            >确 定</el-button
          >
...
import {
  ...
  addYzStPdmmysqS,
  updateYzStPdmmysqS,
  importTemplateYzStPdmmysqS,
  importYzStPdmmysqS,
  exportYzStPdmmysqS,
} from "@/apis/modules/meteorology/YzStPdmmysqS";

const YzStPdmmysqSRef = ref<FormInstance>();
const queryRef = ref<FormInstance>();
const upload = ref<UploadInstance>();
const YzStPdmmysqSList = ref([]);
...
const getList = async () => {
  ...
  const res: any = await listYzStPdmmysqS(queryParams);
  YzStPdmmysqSList.value = res.rows;
  if (YzStPdmmysqSList.value.length > 0) {
    YzStPdmmysqSList.value.forEach((item: any) => {
...
const handleAdd = () => {
  ...
  dialogTiltle.value = "添加旬月降水量系列";
};

/** 修改按钮操作 */
const handleUpdate = async (row: any) => {
  resetForm();
  const res: any = await getYzStPdmmysqS(row);
  form.value = res.data;
  dialogOpen.value = true;
  dialogTiltle.value = "修改旬月降水量系列";
};
...
const submitForm = (formEl: FormInstance | undefined) => {
  ...
  formEl.validate(async (valid) => {
    if (valid) {
      if (dialogTiltle.value == "修改旬月降水量系列") {
        const res: any = await updateYzStPdmmysqS(form.value);
        if (res.code == 200) {
          ElMessage({
            type: "success",
            message: "修改成功！",
        ...
        const res: any = await addYzStPdmmysqS(form.value);
        if (res.code == 200) {
          ElMessage({
            type: "success",
            message: "添加成功！",
...
const handleDelete = (row: any) => {
  ...
  ElMessageBox.confirm(
    ...
    const res: any = await delYzStPdmmysqS(params);
    if (res.code == 200) {
      ElMessage({
        type: "success",
        message: "删除成功！",
...
const handleExport = async () => {
  const res = await exportYzStPdmmysqS(queryParams);
  saveAsExcelFile(res, `YzStPdmmysqS_${new Date().getTime()}`);
};
/** 导出模板按钮操作 */
const handleExportTemplate = async () => {
  const res: any = await importTemplateYzStPdmmysqS({});
...
const handleAddFile = async (ev: any) => {
  ...
  const res: any = await importYzStPdmmysqS(formData);
  if (res.code == 200) {
    ElMessage({
      type: "success",
      message: "导入成功！",
