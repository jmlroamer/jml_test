  ...
  <div class="box-container">
    ...
    >
      ...
      <el-form-item label="方案代码" prop="plcd">
        <el-input
          v-model="queryParams.plcd"
          placeholder="请输入方案代码"
          clearable
          @keyup.enter="handleQuery"
        />
      </el-form-item>
      <el-form-item label="依据时间" style="width: 440px">
        <el-date-picker
          v-model="daterangeFYMDH"
          type="datetimerange"
      ...
      <el-form-item label="发布时间" style="width: 440px">
        <el-date-picker
          v-model="daterangeIYMDH"
          type="datetimerange"
          format="YYYY-MM-DD HH:mm:ss"
          date-format="YYYY/MM/DD ddd"
          time-format="HH:mm:ss"
      ...
      <el-form-item label="发生时间" style="width: 440px">
        <el-date-picker
          v-model="daterangeYMDH"
          type="datetimerange"
          format="YYYY-MM-DD HH:mm:ss"
          date-format="YYYY/MM/DD ddd"
          time-format="HH:mm:ss"
    ...
    >
      ...
      <el-table-column label="依据时间" align="center" prop="fymdh" />
      <el-table-column label="发布时间" align="center" prop="iymdh" />
      <el-table-column label="发生时间" align="center" prop="ymdh" />
      <el-table-column label="预报水位" align="center" prop="z" />
      <el-table-column label="预报蓄水量" align="center" prop="w" />
      <el-table-column label="预报出流" align="center" prop="otq" />
      <el-table-column
        label="操作"
        fixed="right"
        width="155"
    ...
    >
      ...
      >
        <el-form-item label="测站编码" prop="stcd">
          <el-input
            :disabled="dialogTiltle == '修改调度预报成果'"
            v-model="form.stcd"
            placeholder="请输入测站编码"
          />
        </el-form-item>
        ...
        <el-form-item label="预报蓄水量" prop="w">
          <el-input v-model="form.w" placeholder="请输入预报蓄水量" />
        </el-form-item>
        <el-form-item label="预报出流" prop="otq">
          <el-input v-model="form.otq" placeholder="请输入预报出流" />
        </el-form-item>
      </el-form>
      <template #footer>
        <div class="dialog-footer">
          <el-button type="primary" @click="submitForm(YzStReglatFRef)"
            >确 定</el-button
          >
...
import {
  ...
  addYzStReglatF,
  updateYzStReglatF,
  importTemplateYzStReglatF,
  importYzStReglatF,
  exportYzStReglatF,
} from "@/apis/modules/meteorology/YzStReglatF";

const YzStReglatFRef = ref<FormInstance>();
const queryRef = ref<FormInstance>();
const upload = ref<UploadInstance>();
const YzStReglatFList = ref([]);
...
const getList = async () => {
  ...
  const res: any = await listYzStReglatF(queryParams);
  YzStReglatFList.value = res.rows;
  if (YzStReglatFList.value.length > 0) {
    YzStReglatFList.value.forEach((item: any) => {
...
const handleAdd = () => {
  ...
  dialogTiltle.value = "添加调度预报成果";
};

/** 修改按钮操作 */
const handleUpdate = async (row: any) => {
  resetForm();
  const res: any = await getYzStReglatF(row);
  form.value = res.data;
  dialogOpen.value = true;
  dialogTiltle.value = "修改调度预报成果";
};
...
const submitForm = (formEl: FormInstance | undefined) => {
  ...
  formEl.validate(async (valid) => {
    if (valid) {
      if (dialogTiltle.value == "修改调度预报成果") {
        const res: any = await updateYzStReglatF(form.value);
        if (res.code == 200) {
          ElMessage({
            type: "success",
            message: "修改成功！",
        ...
        const res: any = await addYzStReglatF(form.value);
        if (res.code == 200) {
          ElMessage({
            type: "success",
            message: "添加成功！",
...
const handleDelete = (row: any) => {
  ...
  ElMessageBox.confirm(
    ...
    const res: any = await delYzStReglatF(params);
    if (res.code == 200) {
      ElMessage({
        type: "success",
        message: "删除成功！",
...
const handleExport = async () => {
  const res = await exportYzStReglatF(queryParams);
  saveAsExcelFile(res, `YzStReglatF_${new Date().getTime()}`);
};
/** 导出模板按钮操作 */
const handleExportTemplate = async () => {
  const res: any = await importTemplateYzStReglatF({});
...
const handleAddFile = async (ev: any) => {
  ...
  const res: any = await importYzStReglatF(formData);
  if (res.code == 200) {
    ElMessage({
      type: "success",
      message: "导入成功！",
