  ...
  <div class="box-container">
    ...
    >
      ...
      >
        <el-form-item label="测站编码" prop="stcd">
          <el-input
            :disabled="dialogTiltle == '修改河道水情极值'"
            v-model="form.stcd"
            placeholder="请输入测站编码"
          />
        </el-form-item>
      ...
      <template #footer>
        <div class="dialog-footer">
          <el-button type="primary" @click="submitForm(YzStRvevsRRef)"
            >确 定</el-button
          >
...
import {
  ...
  addYzStRvevsR,
  updateYzStRvevsR,
  importTemplateYzStRvevsR,
  importYzStRvevsR,
  exportYzStRvevsR,
} from "@/apis/modules/meteorology/YzStRvevsR";

const YzStRvevsRRef = ref<FormInstance>();
const queryRef = ref<FormInstance>();
const upload = ref<UploadInstance>();
const YzStRvevsRList = ref([]);
...
const getList = async () => {
  ...
  const res: any = await listYzStRvevsR(queryParams);
  YzStRvevsRList.value = res.rows;
  if (YzStRvevsRList.value.length > 0) {
    YzStRvevsRList.value.forEach((item: any) => {
...
const handleAdd = () => {
  ...
  dialogTiltle.value = "添加河道水情极值";
};

/** 修改按钮操作 */
const handleUpdate = async (row: any) => {
  resetForm();
  const res: any = await getYzStRvevsR(row);
  form.value = res.data;
  dialogOpen.value = true;
  dialogTiltle.value = "修改河道水情极值";
};
...
const submitForm = (formEl: FormInstance | undefined) => {
  ...
  formEl.validate(async (valid) => {
    if (valid) {
      if (dialogTiltle.value == "修改河道水情极值") {
        const res: any = await updateYzStRvevsR(form.value);
        if (res.code == 200) {
          ElMessage({
            type: "success",
            message: "修改成功！",
        ...
        const res: any = await addYzStRvevsR(form.value);
        if (res.code == 200) {
          ElMessage({
            type: "success",
            message: "添加成功！",
...
const handleDelete = (row: any) => {
  ...
  ElMessageBox.confirm(
    ...
    const res: any = await delYzStRvevsR(params);
    if (res.code == 200) {
      ElMessage({
        type: "success",
        message: "删除成功！",
...
const handleExport = async () => {
  const res = await exportYzStRvevsR(queryParams);
  saveAsExcelFile(res, `YzStRvevsR_${new Date().getTime()}`);
};
/** 导出模板按钮操作 */
const handleExportTemplate = async () => {
  const res: any = await importTemplateYzStRvevsR({});
...
const handleAddFile = async (ev: any) => {
  ...
  const res: any = await importYzStRvevsR(formData);
  if (res.code == 200) {
    ElMessage({
      type: "success",
      message: "导入成功！",
