<template>
  <div class="box-container">
    ...
    >
      ...
      <el-table-column label="多年日平均水位" align="center" prop="mydavz" />
      <el-table-column label="多年日平均流量" align="center" prop="mydavq" />
    ...
    >
      ...
      >
        <el-form-item label="测站编码" prop="stcd">
          <el-input
            :disabled="dialogTiltle == '修改水位流量多年日平均统计'"
            v-model="form.stcd"
            placeholder="请输入测站编码"
          />
        </el-form-item>
        ...
        <el-form-item label="多年日平均水位" prop="mydavz">
          <el-input v-model="form.mydavz" placeholder="请输入多年日平均水位" />
        </el-form-item>
        <el-form-item label="多年日平均流量" prop="mydavq">
          <el-input v-model="form.mydavq" placeholder="请输入多年日平均流量" />
        </el-form-item>
      ...
      <template #footer>
        <div class="dialog-footer">
          <el-button type="primary" @click="submitForm(YzStRvdaymyavSRef)"
            >确 定</el-button
          >
...
import {
  ...
  delYzStRvdaymyavS,
  addYzStRvdaymyavS,
  updateYzStRvdaymyavS,
  importTemplateYzStRvdaymyavS,
  importYzStRvdaymyavS,
  exportYzStRvdaymyavS,
} from "@/apis/modules/meteorology/YzStRvdaymyavS";

const YzStRvdaymyavSRef = ref<FormInstance>();
const queryRef = ref<FormInstance>();
const upload = ref<UploadInstance>();
const YzStRvdaymyavSList = ref([]);
...
const getList = async () => {
  ...
  const res: any = await listYzStRvdaymyavS(queryParams);
  YzStRvdaymyavSList.value = res.rows;
  if (YzStRvdaymyavSList.value.length > 0) {
    YzStRvdaymyavSList.value.forEach((item: any) => {
...
const handleAdd = () => {
  ...
  dialogTiltle.value = "添加水位流量多年日平均统计";
};

/** 修改按钮操作 */
const handleUpdate = async (row: any) => {
  resetForm();
  const res: any = await getYzStRvdaymyavS(row);
  form.value = res.data;
  dialogOpen.value = true;
  dialogTiltle.value = "修改水位流量多年日平均统计";
};
...
const submitForm = (formEl: FormInstance | undefined) => {
  ...
  formEl.validate(async (valid) => {
    if (valid) {
      if (dialogTiltle.value == "修改水位流量多年日平均统计") {
        const res: any = await updateYzStRvdaymyavS(form.value);
        if (res.code == 200) {
          ElMessage({
            type: "success",
            message: "修改成功！",
        ...
        const res: any = await addYzStRvdaymyavS(form.value);
        if (res.code == 200) {
          ElMessage({
            type: "success",
            message: "添加成功！",
...
const handleDelete = (row: any) => {
  ...
  ElMessageBox.confirm(
    "是否确认删除水位流量多年日平均统计编号为" + _stcds + "的数据项？",
    "删除确认框",
    {
    ...
    const res: any = await delYzStRvdaymyavS(params);
    if (res.code == 200) {
      ElMessage({
        type: "success",
        message: "删除成功！",
...
const handleExport = async () => {
  const res = await exportYzStRvdaymyavS(queryParams);
  saveAsExcelFile(res, `YzStRvdaymyavS_${new Date().getTime()}`);
};
/** 导出模板按钮操作 */
const handleExportTemplate = async () => {
  const res: any = await importTemplateYzStRvdaymyavS({});
...
const handleAddFile = async (ev: any) => {
  ...
  const res: any = await importYzStRvdaymyavS(formData);
  if (res.code == 200) {
    ElMessage({
      type: "success",
      message: "导入成功！",
