  ...
  <div class="box-container">
    ...
    >
      ...
      >
        <el-form-item label="测站编码" prop="stcd">
          <el-input
            :disabled="dialogTiltle == '修改河道水情实时'"
            v-model="form.stcd"
            placeholder="请输入测站编码"
          />
        </el-form-item>
      ...
      <template #footer>
        <div class="dialog-footer">
          <el-button type="primary" @click="submitForm(YzStRiverRRealRef)"
            >确 定</el-button
          >
...
import {
  ...
  delYzStRiverRReal,
  addYzStRiverRReal,
  updateYzStRiverRReal,
  importTemplateYzStRiverRReal,
  importYzStRiverRReal,
  exportYzStRiverRReal,
} from "@/apis/modules/meteorology/YzStRiverRReal";

const YzStRiverRRealRef = ref<FormInstance>();
const queryRef = ref<FormInstance>();
const upload = ref<UploadInstance>();
const YzStRiverRRealList = ref([]);
...
const getList = async () => {
  ...
  const res: any = await listYzStRiverRReal(queryParams);
  YzStRiverRRealList.value = res.rows;
  if (YzStRiverRRealList.value.length > 0) {
    YzStRiverRRealList.value.forEach((item: any) => {
...
const handleAdd = () => {
  ...
  dialogTiltle.value = "添加河道水情实时";
};

/** 修改按钮操作 */
const handleUpdate = async (row: any) => {
  resetForm();
  const res: any = await getYzStRiverRReal(row);
  form.value = res.data;
  dialogOpen.value = true;
  dialogTiltle.value = "修改河道水情实时";
};
...
const submitForm = (formEl: FormInstance | undefined) => {
  ...
  formEl.validate(async (valid) => {
    if (valid) {
      if (dialogTiltle.value == "修改河道水情实时") {
        const res: any = await updateYzStRiverRReal(form.value);
        if (res.code == 200) {
          ElMessage({
            type: "success",
            message: "修改成功！",
        ...
        const res: any = await addYzStRiverRReal(form.value);
        if (res.code == 200) {
          ElMessage({
            type: "success",
            message: "添加成功！",
...
const handleDelete = (row: any) => {
  ...
  ElMessageBox.confirm(
    ...
    const res: any = await delYzStRiverRReal(params);
    if (res.code == 200) {
      ElMessage({
        type: "success",
        message: "删除成功！",
...
const handleExport = async () => {
  const res = await exportYzStRiverRReal(queryParams);
  saveAsExcelFile(res, `YzStRiverRReal_${new Date().getTime()}`);
};
/** 导出模板按钮操作 */
const handleExportTemplate = async () => {
  const res: any = await importTemplateYzStRiverRReal({});
...
const handleAddFile = async (ev: any) => {
  ...
  const res: any = await importYzStRiverRReal(formData);
  if (res.code == 200) {
    ElMessage({
      type: "success",
      message: "导入成功！",
