  ...
  <div class="box-container">
    ...
    >
      ...
      >
        <el-form-item label="测站编码" prop="stcd">
          <el-input
            :disabled="dialogTiltle == '修改水位流量旬月极值系列'"
            v-model="form.stcd"
            placeholder="请输入测站编码"
          />
        </el-form-item>
      ...
      <template #footer>
        <div class="dialog-footer">
          <el-button type="primary" @click="submitForm(YzStRvdmevsqSRef)"
            >确 定</el-button
          >
...
import {
  ...
  addYzStRvdmevsqS,
  updateYzStRvdmevsqS,
  importTemplateYzStRvdmevsqS,
  importYzStRvdmevsqS,
  exportYzStRvdmevsqS,
} from "@/apis/modules/meteorology/YzStRvdmevsqS";

const YzStRvdmevsqSRef = ref<FormInstance>();
const queryRef = ref<FormInstance>();
const upload = ref<UploadInstance>();
const YzStRvdmevsqSList = ref([]);
...
const getList = async () => {
  ...
  const res: any = await listYzStRvdmevsqS(queryParams);
  YzStRvdmevsqSList.value = res.rows;
  if (YzStRvdmevsqSList.value.length > 0) {
    YzStRvdmevsqSList.value.forEach((item: any) => {
...
const handleAdd = () => {
  ...
  dialogTiltle.value = "添加水位流量旬月极值系列";
};

/** 修改按钮操作 */
const handleUpdate = async (row: any) => {
  resetForm();
  const res: any = await getYzStRvdmevsqS(row);
  form.value = res.data;
  dialogOpen.value = true;
  dialogTiltle.value = "修改水位流量旬月极值系列";
};
...
const submitForm = (formEl: FormInstance | undefined) => {
  ...
  formEl.validate(async (valid) => {
    if (valid) {
      if (dialogTiltle.value == "修改水位流量旬月极值系列") {
        const res: any = await updateYzStRvdmevsqS(form.value);
        if (res.code == 200) {
          ElMessage({
            type: "success",
            message: "修改成功！",
        ...
        const res: any = await addYzStRvdmevsqS(form.value);
        if (res.code == 200) {
          ElMessage({
            type: "success",
            message: "添加成功！",
...
const handleDelete = (row: any) => {
  ...
  ElMessageBox.confirm(
    "是否确认删除水位流量旬月极值系列编号为" + _stcds + "的数据项？",
    "删除确认框",
    {
    ...
    const res: any = await delYzStRvdmevsqS(params);
    if (res.code == 200) {
      ElMessage({
        type: "success",
        message: "删除成功！",
...
const handleExport = async () => {
  const res = await exportYzStRvdmevsqS(queryParams);
  saveAsExcelFile(res, `YzStRvdmevsqS_${new Date().getTime()}`);
};
/** 导出模板按钮操作 */
const handleExportTemplate = async () => {
  const res: any = await importTemplateYzStRvdmevsqS({});
...
const handleAddFile = async (ev: any) => {
  ...
  const res: any = await importYzStRvdmevsqS(formData);
  if (res.code == 200) {
    ElMessage({
      type: "success",
      message: "导入成功！",
