<template>
  <div class="box-container">
    ...
    >
      ...
      <el-table-column label="累计蒸发量" align="center" prop="acce" />
      <el-table-column
        label="操作"
        fixed="right"
        width="155"
    ...
    >
      ...
      >
        ...
        <el-form-item label="累计蒸发量" prop="acce">
          <el-input v-model="form.acce" placeholder="请输入累计蒸发量" />
        </el-form-item>
      </el-form>
      <template #footer>
        <div class="dialog-footer">
          <el-button type="primary" @click="submitForm(YzStEstatRRef)"
            >确 定</el-button
          >
...
import {
  ...
  addYzStEstatR,
  updateYzStEstatR,
  importTemplateYzStEstatR,
  importYzStEstatR,
  exportYzStEstatR,
} from "@/apis/modules/meteorology/YzStEstatR";

const YzStEstatRRef = ref<FormInstance>();
const queryRef = ref<FormInstance>();
const upload = ref<UploadInstance>();
const YzStEstatRList = ref([]);
...
const getList = async () => {
  ...
  const res: any = await listYzStEstatR(queryParams);
  YzStEstatRList.value = res.rows;
  if (YzStEstatRList.value.length > 0) {
    YzStEstatRList.value.forEach((item: any) => {
...
const handleAdd = () => {
  ...
  dialogTiltle.value = "添加蒸发量统计";
};

/** 修改按钮操作 */
const handleUpdate = async (row: any) => {
  resetForm();
  const res: any = await getYzStEstatR(row);
  form.value = res.data;
  dialogOpen.value = true;
  dialogTiltle.value = "修改蒸发量统计";
};
...
const submitForm = (formEl: FormInstance | undefined) => {
  ...
  formEl.validate(async (valid) => {
    if (valid) {
      if (dialogTiltle.value == "修改蒸发量统计") {
        const res: any = await updateYzStEstatR(form.value);
        if (res.code == 200) {
          ElMessage({
            type: "success",
            message: "修改成功！",
        ...
        const res: any = await addYzStEstatR(form.value);
        if (res.code == 200) {
          ElMessage({
            type: "success",
            message: "添加成功！",
...
const handleDelete = (row: any) => {
  ...
  ElMessageBox.confirm(
    ...
    const res: any = await delYzStEstatR(params);
    if (res.code == 200) {
      ElMessage({
        type: "success",
        message: "删除成功！",
...
const handleExport = async () => {
  const res = await exportYzStEstatR(queryParams);
  saveAsExcelFile(res, `YzStEstatR_${new Date().getTime()}`);
};
/** 导出模板按钮操作 */
const handleExportTemplate = async () => {
  const res: any = await importTemplateYzStEstatR({});
...
const handleAddFile = async (ev: any) => {
  ...
  const res: any = await importYzStEstatR(formData);
  if (res.code == 200) {
    ElMessage({
      type: "success",
      message: "导入成功！",
