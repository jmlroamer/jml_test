  ...
  <div class="box-container">
    ...
    >
      ...
      >
        ...
        <el-form-item label="多年平均旬月降水量" prop="mymavp">
          <el-input
            v-model="form.mymavp"
            placeholder="请输入多年平均旬月降水量"
          />
        </el-form-item>
      ...
      <template #footer>
        <div class="dialog-footer">
          <el-button type="primary" @click="submitForm(YzStPdmmyavSRef)"
            >确 定</el-button
          >
...
import {
  ...
  addYzStPdmmyavS,
  updateYzStPdmmyavS,
  importTemplateYzStPdmmyavS,
  importYzStPdmmyavS,
  exportYzStPdmmyavS,
} from "@/apis/modules/meteorology/YzStPdmmyavS";

const YzStPdmmyavSRef = ref<FormInstance>();
const queryRef = ref<FormInstance>();
const upload = ref<UploadInstance>();
const YzStPdmmyavSList = ref([]);
...
const getList = async () => {
  ...
  const res: any = await listYzStPdmmyavS(queryParams);
  YzStPdmmyavSList.value = res.rows;
  if (YzStPdmmyavSList.value.length > 0) {
    YzStPdmmyavSList.value.forEach((item: any) => {
...
const handleUpdate = async (row: any) => {
  resetForm();
  const res: any = await getYzStPdmmyavS(row);
  form.value = res.data;
  dialogOpen.value = true;
  ...
  formEl.validate(async (valid) => {
    if (valid) {
      if (dialogTiltle.value == "修改旬月降水量系列") {
        const res: any = await updateYzStPdmmyavS(form.value);
        if (res.code == 200) {
          ElMessage({
            type: "success",
            message: "修改成功！",
        ...
        const res: any = await addYzStPdmmyavS(form.value);
        if (res.code == 200) {
          ElMessage({
            type: "success",
            message: "添加成功！",
...
const handleDelete = (row: any) => {
  ...
  ElMessageBox.confirm(
    ...
    const res: any = await delYzStPdmmyavS(params);
    if (res.code == 200) {
      ElMessage({
        type: "success",
        message: "删除成功！",
...
const handleExport = async () => {
  const res = await exportYzStPdmmyavS(queryParams);
  saveAsExcelFile(res, `YzStPdmmyavS_${new Date().getTime()}`);
};
/** 导出模板按钮操作 */
const handleExportTemplate = async () => {
  const res: any = await importTemplateYzStPdmmyavS({});
...
const handleAddFile = async (ev: any) => {
  ...
  const res: any = await importYzStPdmmyavS(formData);
  if (res.code == 200) {
    ElMessage({
      type: "success",
      message: "导入成功！",
