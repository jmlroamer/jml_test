<template>
  <div class="box-container">
    ...
    >
      ...
      <el-table-column label="多年平均日降水量" align="center" prop="mydavp" />
    ...
    >
      ...
      >
        <el-form-item label="测站编码" prop="stcd">
          <el-input
            :disabled="dialogTiltle == '修改日降水量均值'"
            v-model="form.stcd"
            placeholder="请输入测站编码"
          />
        </el-form-item>
        ...
        <el-form-item label="多年平均日降水量" prop="mydavp">
          <el-input
            v-model="form.mydavp"
            placeholder="请输入多年平均日降水量"
          />
        </el-form-item>
      ...
      <template #footer>
        <div class="dialog-footer">
          <el-button type="primary" @click="submitForm(YzStPddmyavSRef)"
            >确 定</el-button
          >
...
import {
  ...
  addYzStPddmyavS,
  updateYzStPddmyavS,
  importTemplateYzStPddmyavS,
  importYzStPddmyavS,
  exportYzStPddmyavS,
} from "@/apis/modules/meteorology/YzStPddmyavS";

const YzStPddmyavSRef = ref<FormInstance>();
const queryRef = ref<FormInstance>();
const upload = ref<UploadInstance>();
const YzStPddmyavSList = ref([]);
...
const getList = async () => {
  ...
  const res: any = await listYzStPddmyavS(queryParams);
  YzStPddmyavSList.value = res.rows;
  if (YzStPddmyavSList.value.length > 0) {
    YzStPddmyavSList.value.forEach((item: any) => {
...
const handleAdd = () => {
  ...
  dialogTiltle.value = "添加日降水量均值";
};

/** 修改按钮操作 */
const handleUpdate = async (row: any) => {
  resetForm();
  const res: any = await getYzStPddmyavS(row);
  form.value = res.data;
  dialogOpen.value = true;
  dialogTiltle.value = "修改日降水量均值";
};
...
const submitForm = (formEl: FormInstance | undefined) => {
  ...
  formEl.validate(async (valid) => {
    if (valid) {
      if (dialogTiltle.value == "修改日降水量均值") {
        const res: any = await updateYzStPddmyavS(form.value);
        if (res.code == 200) {
          ElMessage({
            type: "success",
            message: "修改成功！",
        ...
        const res: any = await addYzStPddmyavS(form.value);
        if (res.code == 200) {
          ElMessage({
            type: "success",
            message: "添加成功！",
...
const handleDelete = (row: any) => {
  ...
  ElMessageBox.confirm(
    ...
    const res: any = await delYzStPddmyavS(params);
    if (res.code == 200) {
      ElMessage({
        type: "success",
        message: "删除成功！",
...
const handleExport = async () => {
  const res = await exportYzStPddmyavS(queryParams);
  saveAsExcelFile(res, `YzStPddmyavS_${new Date().getTime()}`);
};
/** 导出模板按钮操作 */
const handleExportTemplate = async () => {
  const res: any = await importTemplateYzStPddmyavS({});
...
const handleAddFile = async (ev: any) => {
  ...
  const res: any = await importYzStPddmyavS(formData);
  if (res.code == 200) {
    ElMessage({
      type: "success",
      message: "导入成功！",
