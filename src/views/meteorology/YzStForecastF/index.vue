  ...
  <div class="box-container">
    ...
    >
      ...
      >
        <el-form-item label="测站编码" prop="stcd">
          <el-input
            :disabled="dialogTiltle == '修改水情预报成果'"
            v-model="form.stcd"
            placeholder="请输入测站编码"
          />
        </el-form-item>
      ...
      <template #footer>
        <div class="dialog-footer">
          <el-button type="primary" @click="submitForm(YzStForecastFRef)"
            >确 定</el-button
          >
...
import {
  ...
  addYzStForecastF,
  updateYzStForecastF,
  importTemplateYzStForecastF,
  importYzStForecastF,
  exportYzStForecastF,
} from "@/apis/modules/meteorology/YzStForecastF";

const YzStForecastFRef = ref<FormInstance>();
const queryRef = ref<FormInstance>();
const upload = ref<UploadInstance>();
const YzStForecastFList = ref([]);
...
const getList = async () => {
  ...
  const res: any = await listYzStForecastF(queryParams);
  YzStForecastFList.value = res.rows;
  if (YzStForecastFList.value.length > 0) {
    YzStForecastFList.value.forEach((item: any) => {
...
const handleUpdate = async (row: any) => {
  resetForm();
  const res: any = await getYzStForecastF(row);
  form.value = res.data;
  dialogOpen.value = true;
  ...
  formEl.validate(async (valid) => {
    if (valid) {
      if (dialogTiltle.value == "修改水情预报成果") {
        const res: any = await updateYzStForecastF(form.value);
        if (res.code == 200) {
          ElMessage({
            type: "success",
            message: "修改成功！",
        ...
        const res: any = await addYzStForecastF(form.value);
        if (res.code == 200) {
          ElMessage({
            type: "success",
            message: "添加成功！",
...
const handleDelete = (row: any) => {
  ...
  ElMessageBox.confirm(
    ...
    const res: any = await delYzStForecastF(params);
    if (res.code == 200) {
      ElMessage({
        type: "success",
        message: "删除成功！",
...
const handleExport = async () => {
  const res = await exportYzStForecastF(queryParams);
  saveAsExcelFile(res, `YzStForecastF_${new Date().getTime()}`);
};
/** 导出模板按钮操作 */
const handleExportTemplate = async () => {
  const res: any = await importTemplateYzStForecastF({});
...
const handleAddFile = async (ev: any) => {
  ...
  const res: any = await importYzStForecastF(formData);
  if (res.code == 200) {
    ElMessage({
      type: "success",
      message: "导入成功！",
