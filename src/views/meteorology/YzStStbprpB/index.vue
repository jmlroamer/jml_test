<template>
  <div class="box-container">
    ...
    >
      ...
      <el-table-column label="桩号" align="center" prop="bnch" />
      <el-table-column label="所属项目名称id" align="center" prop="projectid" />
      <el-table-column label="0: 未删除 1: 删除" align="center" prop="del" />
      <el-table-column label="区域编码" align="center" prop="region" />
      <el-table-column label="RTU编码" align="center" prop="rtuid" />
    ...
    >
      ...
      >
        <el-form-item label="测站编码" prop="stcd">
          <el-input
            :disabled="dialogTiltle == '修改待确认站点删除'"
            v-model="form.stcd"
            placeholder="请输入测站编码"
          />
        </el-form-item>
        ...
        <el-form-item label="桩号" prop="bnch">
          <el-input v-model="form.bnch" placeholder="请输入桩号" />
        </el-form-item>
        <el-form-item label="所属项目名称id" prop="projectid">
          <el-input
            v-model="form.projectid"
            placeholder="请输入所属项目名称id"
          />
        </el-form-item>
        <el-form-item label="0: 未删除 1: 删除" prop="del">
          <el-input v-model="form.del" placeholder="请输入0: 未删除 1: 删除" />
        </el-form-item>
        <el-form-item label="区域编码" prop="region">
          <el-input v-model="form.region" placeholder="请输入区域编码" />
        </el-form-item>
        <el-form-item label="RTU编码" prop="rtuid">
          <el-input v-model="form.rtuid" placeholder="请输入RTU编码" />
        </el-form-item>
      ...
      <template #footer>
        <div class="dialog-footer">
          <el-button type="primary" @click="submitForm(YzStStbprpBRef)"
            >确 定</el-button
          >
...
import {
  ...
  addYzStStbprpB,
  updateYzStStbprpB,
  importTemplateYzStStbprpB,
  importYzStStbprpB,
  exportYzStStbprpB,
} from "@/apis/modules/meteorology/YzStStbprpB";

const YzStStbprpBRef = ref<FormInstance>();
const queryRef = ref<FormInstance>();
const upload = ref<UploadInstance>();
const YzStStbprpBList = ref([]);
...
const getList = async () => {
  ...
  const res: any = await listYzStStbprpB(queryParams);
  YzStStbprpBList.value = res.rows;
  if (YzStStbprpBList.value.length > 0) {
    YzStStbprpBList.value.forEach((item: any) => {
...
const handleAdd = () => {
  ...
  dialogTiltle.value = "添加待确认站点删除";
};

/** 修改按钮操作 */
const handleUpdate = async (row: any) => {
  resetForm();
  const res: any = await getYzStStbprpB(row);
  form.value = res.data;
  dialogOpen.value = true;
  dialogTiltle.value = "修改待确认站点删除";
};
...
const submitForm = (formEl: FormInstance | undefined) => {
  ...
  formEl.validate(async (valid) => {
    if (valid) {
      if (dialogTiltle.value == "修改待确认站点删除") {
        const res: any = await updateYzStStbprpB(form.value);
        if (res.code == 200) {
          ElMessage({
            type: "success",
            message: "修改成功！",
        ...
        const res: any = await addYzStStbprpB(form.value);
        if (res.code == 200) {
          ElMessage({
            type: "success",
            message: "添加成功！",
...
const handleDelete = (row: any) => {
  ...
  ElMessageBox.confirm(
    ...
    const res: any = await delYzStStbprpB(params);
    if (res.code == 200) {
      ElMessage({
        type: "success",
        message: "删除成功！",
...
const handleExport = async () => {
  const res = await exportYzStStbprpB(queryParams);
  saveAsExcelFile(res, `YzStStbprpB_${new Date().getTime()}`);
};
/** 导出模板按钮操作 */
const handleExportTemplate = async () => {
  const res: any = await importTemplateYzStStbprpB({});
...
const handleAddFile = async (ev: any) => {
  ...
  const res: any = await importYzStStbprpB(formData);
  if (res.code == 200) {
    ElMessage({
      type: "success",
      message: "导入成功！",
