<template>
  <div class="box-container">
    ...
    >
      ...
      <el-table-column label="累计开采量" align="center" prop="accgdw" />
      <el-table-column
        label="操作"
        fixed="right"
        width="155"
    ...
    >
      ...
      >
        <el-form-item label="测站编码" prop="stcd">
          <el-input
            :disabled="dialogTiltle == '修改地下水开采量统计'"
            v-model="form.stcd"
            placeholder="请输入测站编码"
          />
        </el-form-item>
        ...
        <el-form-item label="累计开采量" prop="accgdw">
          <el-input v-model="form.accgdw" placeholder="请输入累计开采量" />
        </el-form-item>
      </el-form>
      <template #footer>
        <div class="dialog-footer">
          <el-button type="primary" @click="submitForm(YzStWgrwstatRRef)"
            >确 定</el-button
          >
...
import {
  ...
  addYzStWgrwstatR,
  updateYzStWgrwstatR,
  importTemplateYzStWgrwstatR,
  importYzStWgrwstatR,
  exportYzStWgrwstatR,
} from "@/apis/modules/meteorology/YzStWgrwstatR";

const YzStWgrwstatRRef = ref<FormInstance>();
const queryRef = ref<FormInstance>();
const upload = ref<UploadInstance>();
const YzStWgrwstatRList = ref([]);
...
const getList = async () => {
  ...
  const res: any = await listYzStWgrwstatR(queryParams);
  YzStWgrwstatRList.value = res.rows;
  if (YzStWgrwstatRList.value.length > 0) {
    YzStWgrwstatRList.value.forEach((item: any) => {
...
const handleAdd = () => {
  ...
  dialogTiltle.value = "添加地下水开采量统计";
};

/** 修改按钮操作 */
const handleUpdate = async (row: any) => {
  resetForm();
  const res: any = await getYzStWgrwstatR(row);
  form.value = res.data;
  dialogOpen.value = true;
  dialogTiltle.value = "修改地下水开采量统计";
};
...
const submitForm = (formEl: FormInstance | undefined) => {
  ...
  formEl.validate(async (valid) => {
    if (valid) {
      if (dialogTiltle.value == "修改地下水开采量统计") {
        const res: any = await updateYzStWgrwstatR(form.value);
        if (res.code == 200) {
          ElMessage({
            type: "success",
            message: "修改成功！",
        ...
        const res: any = await addYzStWgrwstatR(form.value);
        if (res.code == 200) {
          ElMessage({
            type: "success",
            message: "添加成功！",
...
const handleDelete = (row: any) => {
  ...
  ElMessageBox.confirm(
    ...
    const res: any = await delYzStWgrwstatR(params);
    if (res.code == 200) {
      ElMessage({
        type: "success",
        message: "删除成功！",
...
const handleExport = async () => {
  const res = await exportYzStWgrwstatR(queryParams);
  saveAsExcelFile(res, `YzStWgrwstatR_${new Date().getTime()}`);
};
/** 导出模板按钮操作 */
const handleExportTemplate = async () => {
  const res: any = await importTemplateYzStWgrwstatR({});
...
const handleAddFile = async (ev: any) => {
  ...
  const res: any = await importYzStWgrwstatR(formData);
  if (res.code == 200) {
    ElMessage({
      type: "success",
      message: "导入成功！",
