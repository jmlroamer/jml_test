    ...
    >
      ...
      >
        ...
        <el-form-item label="蒸发器类型" prop="eptp">
          <el-input v-model="form.eptp" placeholder="请输入蒸发器类型" />
        </el-form-item>
        <el-form-item label="日蒸发量" prop="dye">
          <el-input v-model="form.dye" placeholder="请输入日蒸发量" />
        </el-form-item>
      </el-form>
      <template #footer>
        <div class="dialog-footer">
          <el-button type="primary" @click="submitForm(YzStDayevRRef)"
            >确 定</el-button
          >
...
import {
  ...
  addYzStDayevR,
  updateYzStDayevR,
  importTemplateYzStDayevR,
  importYzStDayevR,
  exportYzStDayevR,
} from "@/apis/modules/meteorology/YzStDayevR";

const YzStDayevRRef = ref<FormInstance>();
const queryRef = ref<FormInstance>();
const upload = ref<UploadInstance>();
const YzStDayevRList = ref([]);
...
const getList = async () => {
  ...
  if (YzStDayevRList.value.length > 0) {
    YzStDayevRList.value.forEach((item: any) => {
...
const handleAdd = () => {
  ...
  dialogTiltle.value = "添加日蒸发量";
};

/** 修改按钮操作 */
const handleUpdate = async (row: any) => {
  resetForm();
  const res: any = await getYzStDayevR(row);
  form.value = res.data;
  dialogOpen.value = true;
  dialogTiltle.value = "修改日蒸发量";
};
...
const handleDelete = (row: any) => {
  ...
  ElMessageBox.confirm(
    ...
    const res: any = await delYzStDayevR(params);
    if (res.code == 200) {
      ElMessage({
        type: "success",
        message: "删除成功！",
...
const handleExport = async () => {
  ...
  saveAsExcelFile(res, `YzStDayevR_${new Date().getTime()}`);
};
/** 导出模板按钮操作 */
const handleExportTemplate = async () => {
  const res: any = await importTemplateYzStDayevR({});
...
const handleAddFile = async (ev: any) => {
  ...
  const res: any = await importYzStDayevR(formData);
  if (res.code == 200) {
    ElMessage({
      type: "success",
      message: "导入成功！",
