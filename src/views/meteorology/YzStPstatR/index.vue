    ...
    >
      ...
      <template #footer>
        <div class="dialog-footer">
          <el-button type="primary" @click="submitForm(YzStPstatRRef)"
            >确 定</el-button
          >
...
import {
  ...
  addYzStPstatR,
  updateYzStPstatR,
  importTemplateYzStPstatR,
  importYzStPstatR,
  exportYzStPstatR,
} from "@/apis/modules/meteorology/YzStPstatR";

const YzStPstatRRef = ref<FormInstance>();
const queryRef = ref<FormInstance>();
const upload = ref<UploadInstance>();
const YzStPstatRList = ref([]);
...
const getList = async () => {
  ...
  const res: any = await listYzStPstatR(queryParams);
  YzStPstatRList.value = res.rows;
  if (YzStPstatRList.value.length > 0) {
    YzStPstatRList.value.forEach((item: any) => {
...
const handleAdd = () => {
  ...
  dialogTiltle.value = "添加降水量统计";
};

/** 修改按钮操作 */
const handleUpdate = async (row: any) => {
  resetForm();
  const res: any = await getYzStPstatR(row);
  form.value = res.data;
  dialogOpen.value = true;
  dialogTiltle.value = "修改降水量统计";
};
...
const submitForm = (formEl: FormInstance | undefined) => {
  ...
  formEl.validate(async (valid) => {
    if (valid) {
      if (dialogTiltle.value == "修改降水量统计") {
        const res: any = await updateYzStPstatR(form.value);
        if (res.code == 200) {
          ElMessage({
            type: "success",
            message: "修改成功！",
        ...
        const res: any = await addYzStPstatR(form.value);
        if (res.code == 200) {
          ElMessage({
            type: "success",
            message: "添加成功！",
...
const handleDelete = (row: any) => {
  ...
  ElMessageBox.confirm(
    ...
    const res: any = await delYzStPstatR(params);
    if (res.code == 200) {
      ElMessage({
        type: "success",
        message: "删除成功！",
...
const handleExport = async () => {
  const res = await exportYzStPstatR(queryParams);
  saveAsExcelFile(res, `YzStPstatR_${new Date().getTime()}`);
};
/** 导出模板按钮操作 */
const handleExportTemplate = async () => {
  const res: any = await importTemplateYzStPstatR({});
...
const handleAddFile = async (ev: any) => {
  ...
  const res: any = await importYzStPstatR(formData);
  if (res.code == 200) {
    ElMessage({
      type: "success",
      message: "导入成功！",
