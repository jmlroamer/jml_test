<template>
  <div class="box-container">
    ...
    >
      ...
      <el-table-column label="多年日平均蓄水量" align="center" prop="mydavw" />
    ...
    >
      ...
      >
        <el-form-item label="测站编码" prop="stcd">
          <el-input
            :disabled="dialogTiltle == '修改(湖)蓄水量多年日均值统计'"
            v-model="form.stcd"
            placeholder="请输入测站编码"
          />
        </el-form-item>
        ...
        <el-form-item label="多年日平均蓄水量" prop="mydavw">
          <el-input
            v-model="form.mydavw"
            placeholder="请输入多年日平均蓄水量"
          />
        </el-form-item>
      ...
      <template #footer>
        <div class="dialog-footer">
          <el-button type="primary" @click="submitForm(YzStRsvrmyavSRef)"
            >确 定</el-button
          >
...
import {
  ...
  addYzStRsvrmyavS,
  updateYzStRsvrmyavS,
  importTemplateYzStRsvrmyavS,
  importYzStRsvrmyavS,
  exportYzStRsvrmyavS,
} from "@/apis/modules/meteorology/YzStRsvrmyavS";

const YzStRsvrmyavSRef = ref<FormInstance>();
const queryRef = ref<FormInstance>();
const upload = ref<UploadInstance>();
const YzStRsvrmyavSList = ref([]);
...
const getList = async () => {
  ...
  const res: any = await listYzStRsvrmyavS(queryParams);
  YzStRsvrmyavSList.value = res.rows;
  if (YzStRsvrmyavSList.value.length > 0) {
    YzStRsvrmyavSList.value.forEach((item: any) => {
...
const handleAdd = () => {
  ...
  dialogTiltle.value = "添加(湖)蓄水量多年日均值统计";
};

/** 修改按钮操作 */
const handleUpdate = async (row: any) => {
  resetForm();
  const res: any = await getYzStRsvrmyavS(row);
  form.value = res.data;
  dialogOpen.value = true;
  dialogTiltle.value = "修改(湖)蓄水量多年日均值统计";
};
...
const submitForm = (formEl: FormInstance | undefined) => {
  ...
  formEl.validate(async (valid) => {
    if (valid) {
      if (dialogTiltle.value == "修改(湖)蓄水量多年日均值统计") {
        const res: any = await updateYzStRsvrmyavS(form.value);
        if (res.code == 200) {
          ElMessage({
            type: "success",
            message: "修改成功！",
        ...
        const res: any = await addYzStRsvrmyavS(form.value);
        if (res.code == 200) {
          ElMessage({
            type: "success",
            message: "添加成功！",
...
const handleDelete = (row: any) => {
  ...
  ElMessageBox.confirm(
    "是否确认删除(湖)蓄水量多年日均值统计编号为" + _stcds + "的数据项？",
    "删除确认框",
    {
    ...
    const res: any = await delYzStRsvrmyavS(params);
    if (res.code == 200) {
      ElMessage({
        type: "success",
        message: "删除成功！",
...
const handleExport = async () => {
  const res = await exportYzStRsvrmyavS(queryParams);
  saveAsExcelFile(res, `YzStRsvrmyavS_${new Date().getTime()}`);
};
/** 导出模板按钮操作 */
const handleExportTemplate = async () => {
  const res: any = await importTemplateYzStRsvrmyavS({});
...
const handleAddFile = async (ev: any) => {
  ...
  const res: any = await importYzStRsvrmyavS(formData);
  if (res.code == 200) {
    ElMessage({
      type: "success",
      message: "导入成功！",
