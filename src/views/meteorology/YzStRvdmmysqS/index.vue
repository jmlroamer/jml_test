<template>
  <div class="box-container">
    ...
    >
      ...
      <el-table-column label="旬月平均水位" align="center" prop="avz" />
      <el-table-column label="旬月平均流量" align="center" prop="avq" />
      <el-table-column
        label="操作"
        fixed="right"
        width="155"
    ...
    >
      ...
      >
        <el-form-item label="测站编码" prop="stcd">
          <el-input
            :disabled="dialogTiltle == '修改水位流量旬月均值系列'"
            v-model="form.stcd"
            placeholder="请输入测站编码"
          />
        </el-form-item>
        ...
        <el-form-item label="旬月平均水位" prop="avz">
          <el-input v-model="form.avz" placeholder="请输入旬月平均水位" />
        </el-form-item>
        <el-form-item label="旬月平均流量" prop="avq">
          <el-input v-model="form.avq" placeholder="请输入旬月平均流量" />
        </el-form-item>
      ...
      <template #footer>
        <div class="dialog-footer">
          <el-button type="primary" @click="submitForm(YzStRvdmmysqSRef)"
            >确 定</el-button
          >
...
import {
  ...
  addYzStRvdmmysqS,
  updateYzStRvdmmysqS,
  importTemplateYzStRvdmmysqS,
  importYzStRvdmmysqS,
  exportYzStRvdmmysqS,
} from "@/apis/modules/meteorology/YzStRvdmmysqS";

const YzStRvdmmysqSRef = ref<FormInstance>();
const queryRef = ref<FormInstance>();
const upload = ref<UploadInstance>();
const YzStRvdmmysqSList = ref([]);
...
const getList = async () => {
  ...
  const res: any = await listYzStRvdmmysqS(queryParams);
  YzStRvdmmysqSList.value = res.rows;
  if (YzStRvdmmysqSList.value.length > 0) {
    YzStRvdmmysqSList.value.forEach((item: any) => {
...
const handleAdd = () => {
  ...
  dialogTiltle.value = "添加水位流量旬月均值系列";
};

/** 修改按钮操作 */
const handleUpdate = async (row: any) => {
  resetForm();
  const res: any = await getYzStRvdmmysqS(row);
  form.value = res.data;
  dialogOpen.value = true;
  dialogTiltle.value = "修改水位流量旬月均值系列";
};
...
const submitForm = (formEl: FormInstance | undefined) => {
  ...
  formEl.validate(async (valid) => {
    if (valid) {
      if (dialogTiltle.value == "修改水位流量旬月均值系列") {
        const res: any = await updateYzStRvdmmysqS(form.value);
        if (res.code == 200) {
          ElMessage({
            type: "success",
            message: "修改成功！",
        ...
        const res: any = await addYzStRvdmmysqS(form.value);
        if (res.code == 200) {
          ElMessage({
            type: "success",
            message: "添加成功！",
...
const handleDelete = (row: any) => {
  ...
  ElMessageBox.confirm(
    "是否确认删除水位流量旬月均值系列编号为" + _stcds + "的数据项？",
    "删除确认框",
    {
    ...
    const res: any = await delYzStRvdmmysqS(params);
    if (res.code == 200) {
      ElMessage({
        type: "success",
        message: "删除成功！",
...
const handleExport = async () => {
  const res = await exportYzStRvdmmysqS(queryParams);
  saveAsExcelFile(res, `YzStRvdmmysqS_${new Date().getTime()}`);
};
/** 导出模板按钮操作 */
const handleExportTemplate = async () => {
  const res: any = await importTemplateYzStRvdmmysqS({});
...
const handleAddFile = async (ev: any) => {
  ...
  const res: any = await importYzStRvdmmysqS(formData);
  if (res.code == 200) {
    ElMessage({
      type: "success",
      message: "导入成功！",
